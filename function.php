...
{
	$query = "
	SELECT sum(inventory_order_total) as total_order_value FROM inventory_order WHERE payment_status = 'bank transfer' AND inventory_order_status='active'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= ' AND user_id = "'.$_SESSION["user_id"].'"';
...
function get_user_wise_total_order($connect)
{
	...
	SUM(CASE WHEN inventory_order.payment_status = "bank transfer" THEN inventory_order.inventory_order_total ELSE 0 END) AS credit_order_total, 
	...
	<div class="table-responsive">
		<table class="table table-bordered table-striped">
			<tr>
				...
				<th>Total Bank Transfer Order</th>
			</tr>
	';
	...
	foreach($result as $row)
	{
		...
		<tr>
			<td>'.$row['user_name'].'</td>
			<td align="right">N '.$row["order_total"].'</td>
			<td align="right">N '.$row["cash_order_total"].'</td>
			<td align="right">N '.$row["credit_order_total"].'</td>
		</tr>
		';
